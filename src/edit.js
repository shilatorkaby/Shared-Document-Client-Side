	...
	$(() => {
		...
		let endPos;
			...
			input.on("input", (event) => {
				...
				addUpdate(key.token, event.originalEvent.data, end-1,startPos,endPos,docId)
			})
		})	
}
...
const update = (updateData) => {
	...
	if (sessionStorage.getItem('token') != updateData.user && updateData.documentId == documentId ) {
		...
		if(updateData.content == null && updateData.startPos < updateData.endPos){
			text = text.substring(0, updateData.startPos) + text.substring(updateData.endPos, text.length);
		}
		else if(updateData.content == null){
			text = text.substring(0, updateData.position+1) + text.substring(updateData.position+2, text.length);
		}
		else{
